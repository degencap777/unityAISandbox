...
public class BSP : MonoBehaviour
{
	...
	private void SubdivideNode(BTreeNode<BSPPartition> node)
	{
		...
		Debug.Log("[BSP] Subdivided partition");
	}
	...
	private void CombineChildren(BTreeNode<BSPPartition> parent)
	{
		...
		Debug.Log("[BSP] Combined partitions");
	...
	private void AddAgentToChild(BTreeNode<BSPPartition> parent, Agent agent)
	{
		...
		{
			Debug.LogError("[BSP] Subdivide partition failed; agent cannot be inserted into either subdivision");
		}
	}
	...
	private BTreeNode<BSPPartition> FindNodeForPosition(Vector3 position)
	{
		...
		while (current != null)
		{
			...
			{
				Debug.LogError("[BSP] Invalid BSP tree; nodes should have either 0 or 2 children, found a node with 1 child");
				return null;
			}
			else
			{
				...
				{
					Debug.LogErrorFormat("[BSP] No child nodes contain the agent position({0})", position.ToString());
					return null;
				}
			}
		}
